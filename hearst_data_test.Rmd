...
# Variables to pull (these hold across 2020–2024)
acs_vars <- list(
...
# Execute Fetch Loop
# -----------------------------
# This runs the functions above for every year in range
acs_data_list <- lapply(acs_years, get_city_acs_year)
names(acs_data_list) <- acs_years


#----------------------------------------------------------
# CALCULATE DERIVED METRICS & JOIN TO CRIME DATA
# ---------------------------------------------------------
...
# 1. Ensure population is numeric for the math to work
acs_all_years <- acs_all_years %>%
  mutate(population = as.numeric(population))

# 2. Perform the join and calculate rates
offenses_with_acs <- offenses_all_years %>%
  ...
  mutate(
    ...
    homicide_per_1000       = murder_and_nonnegligent_manslaughter / acs_population * 1000
  )
```
...
```{r}
# -----------------------------
# Step 1: Prepare the 2020 vs 2024 comparison (UPDATED)
# -----------------------------
crime_change <- offenses_with_acs %>%
  ...
  mutate(
    # --- NEW: Population Change Calculation ---
    pop_change      = acs_population_2024 - acs_population_2020,
    ...
    pop_category = case_when(
      ...
      acs_population_2024 < 500000 ~ "3. 200k to 500k",
      TRUE                         ~ "4. Over 500k"
    )
  )

# -----------------------------
# Step 2: Function to extract top/bottom 10 (UPDATED)
# -----------------------------
get_top_changes <- function(data, change_col, base_rate_col, direction = "increase") {
  ...
  data %>%
    ...
    ungroup() %>%
    # --- UPDATED: Added population columns to the output ---
    select(
      pop_category, 
      rank, 
      city, 
      state,
      pop_2020   = acs_population_2020,
      pop_2024   = acs_population_2024,
      pop_change = pop_change,
      start_rate = !!sym(col_2020),
      end_rate   = !!sym(col_2024),
      crime_rate_change = !!sym(change_col)
    ) %>%
...
create_crime_map <- function(data, title_text, metric_label) {
  ...
  leaflet(data) %>%
    ...
    addCircleMarkers(
      ...
      "<strong>Change: ", round(crime_rate_change, 1), "</strong>"
    )
    ) %>%
...
# -----------------------------
# Use quantiles to define "High Growth" and "Big Drop" relative to the dataset
...
create_paradox_map <- function(data, title_text, paradox_type = "boom") {
  ...
  # Re-create pop_category (dropped accidentally)
  if(!"pop_category" %in% colnames(data)) {
    message("Re-creating missing pop_category...")
    data <- data %>%
      mutate(pop_category = case_when(
        pop_2024 < 50000  ~ "1. Under 50k",
        pop_2024 < 200000 ~ "2. 50k to 200k",
        pop_2024 < 500000 ~ "3. 200k to 500k",
        TRUE              ~ "4. Over 500k"
      ))
  }
  ...
  leaflet(data) %>%
    ...
    addCircleMarkers(
      ...
      popup = ~paste0(
        ...
        # Use the names from your actual colnames() output
        "Pop (2024): ", scales::comma(pop_2024), "<br/>",
...
clean_map_data <- function(df) {
  df %>%
    mutate(
      across(
        c(pop_2024, pop_change_pct, violent_rate_2020, violent_rate_2024, violent_change_rate),
        ~ as.numeric(as.character(.))
      )
    )
}

map_data_boom <- clean_map_data(map_data_boom)
map_data_turnaround <- clean_map_data(map_data_turnaround)
